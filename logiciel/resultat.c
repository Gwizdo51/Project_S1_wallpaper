...
void enregistrer_resultat(LL_ROULEAU *liste_rouleaux, float *quantite_colle, int *nombre_pots, char *nom_fichier_resultat) {
    ...
    strftime(nom_fichier_resultat, STRING_BUFFER_MAX_SIZE, "%Y%m%d-%H%M%S - resultats.txt", time_now_pointer);
    ...
    strftime(date_heure_humain, STRING_BUFFER_MAX_SIZE, "%c", time_now_pointer);
